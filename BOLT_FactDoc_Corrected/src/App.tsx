...
function App() {
  ...
  const handleAnalyzeDocument = () => {
    setState(prev => ({ ...prev, currentPage: 'Analyse', isAnalyzing: true }));
  };
  ...
  const handleAnalysisComplete = (result: { score: number; type_document: string; anomalies: string[] }) => {
    setState(prev => ({
      ...
      isAnalyzing: false,
      currentPage: result.score >= 80 ? 'Résultat OK' : 'Résultat KO'
    }));
  };
  ...
  const renderCurrentPage = () => {
    switch (state.currentPage) {
      ...
      case 'Analyse':
        return <AnalysisPage onAnalysisComplete={handleAnalysisComplete} />;
